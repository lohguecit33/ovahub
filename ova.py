...
WIDTH = 96
print("═" * WIDTH)
print(" ROBLOX OVA REJOIN - WORKSPACE MONITOR ".center(WIDTH, "═"))
print("═" * WIDTH)
...
for i, (pkg, info) in enumerate(pkgs.items(), 1):
    ...
    pkg_display = pkg if len(pkg) <= 34 else pkg[:31] + "..."
    ...
    if status == "online":
        ...
        status_display = "🔴 WAIT TIMEOUT"
        all_online = False
    elif status == "timeout":
        status_display = "🔴 ONLINE TIMEOUT"
        all_online = False
    elif status == "offline":
